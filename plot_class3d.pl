...
# wjr 
#
#modified 8/22/17 for relion 2.1
#
...
foreach $file (@files) {
   ...
   while ($#data == $#keys) {
      ...
      if ($iteration>0) {for (my $i=1; $i <= $#data; $i++) {print OUT "$data[$i] ";}}
...
#   print OUT2 "set term png\n";
   ...
   $yrange=0; # flag if yrange needs to be set
...
# no longer necessary since resolution is now plotted per model
...
print "\ndata plotted:\n";
shift (@keys);  #first item is filename so remove it from list
